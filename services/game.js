...
function checkPatterns(board, calledSet) {
  ...
  if ([0,1,2,3,4].every(i => marked[i][i])) patterns.push({ type: 'diagonal', index: 0, label: 'Diagonal â†˜' });
  if ([0,1,2,3,4].every(i => marked[i][4-i])) patterns.push({ type: 'diagonal', index: 1, label: 'Diagonal â†™' });
...
async function startGame(roomId) {
  ...
  try {
    ...
    // ** FIX: Start the number caller loop immediately **
...
// ** ADDED MISSING FUNCTION: THE CALLER LOOP **
async function startNumberCaller(roomId) {
  stopNumberCaller(roomId); // Prevent double loops
  ...
  const timer = setInterval(async () => {
    try {
      ...
      // Get the updated cards to send to frontend
      ...
      if (winners.length > 0) {
        ...
        // Broadcast number called to clients
        if (game.broadcastToRoom) {
          game.broadcastToRoom(roomId, 'numberCalled', {
            number: number,
            calledNumbers: calledNumbers,
            calledCount: calledNumbers.length,
            cards: updatedCards // Sending the fresh cards ensures the frontend marks the greens correctly
          });
        }
      }
    } catch (e) {
