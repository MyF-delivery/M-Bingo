...
async function startNumberCaller(roomId) {
  stopNumberCaller(roomId); // Prevent multiple intervals
  ...
  const timer = setInterval(async () => {
    try {
      ...
      if (index >= sequence.length) {
        // No more numbers, finish with no winner
      ...
      // Check winners based on called set
      const calledSet = new Set(calledNumbers.map(Number));
      ...
      for (const p of activePlayers) {
        for (const cardNum of (p.cards || [])) {
          // Fetch each specific card board (or optimize with a single query)
