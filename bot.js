...
// ---------- Admin Callbacks ----------
// Note: We need a separate handler for admin callbacks because they are also 'callback_query' events.
// We can combine them in the same listener above, but to keep it clean we'll add another listener.
// However, since bot.on('callback_query') can have multiple listeners, we'll attach a second one specifically for admin actions.
// The order of listeners matters: the first one that matches will handle it.
// We'll keep the admin callbacks in a separate listener that checks if the data matches admin actions.
// Actually, we can just add the admin case inside the existing switch, but they are different data values.
// Let's add them as additional cases in the same switch, or we can just put them after the previous switch.
// Since the previous switch handles only specific data, the admin ones won't match, so we can add a second listener.
// But to avoid duplication, we can extend the switch with those cases.
// I'll extend the switch to handle admin_* data as well.

// However, we already have a switch above, so we should add the admin cases there.
// Let's modify the existing callback_query to include admin_* cases.
// I'll rewrite the callback section to handle all in one.

// To keep it clean, I'll replace the entire callback_query section with a unified one that includes admin cases.
