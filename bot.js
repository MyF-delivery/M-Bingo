// ============================================================
// M-BINGO TELEGRAM BOT - HARDENED (Never Crashes)
// ============================================================
...
// ** CRITICAL FIX: Safe Send Wrapper **
async function safeSendMessage(chatId, text, options = {}) {
    try {
        await bot.sendMessage(chatId, text, options);
    } catch (error) {
        // If the message fails (bad format, user blocked bot, etc.), log it and keep the bot alive
...
bot.onText(/\/start(?:\s+ref_(\d+))?/, async (msg, match) => {
    ...
    try {
        ...
        if (response.data.user) {
            ...
            await safeSendMessage(chatId, welcomeBack, { parse_mode: 'HTML', ...mainMenu(userId) });
            return;
        }
    } catch (error) {}
...
🔒 <b>Your Telegram ID will be used to securely identify you.</b>
    ...
    await safeSendMessage(chatId, registerMessage, { parse_mode: 'HTML', ...registerKeyboard });
...
bot.on('contact', async (msg) => {
    ...
    if (!contact || contact.user_id != userId) {
        await safeSendMessage(chatId, '❌ Please share your own contact.');
        return;
    }
    ...
    try {
        const response = await axios.get(`${API_URL}/api/users/${userId}`);
        if (response.data.user) {
            await safeSendMessage(chatId, '✅ You are already registered!');
            return;
        }
    } catch (error) {}
    ...
    try {
        const user = await getOrCreateUser(userId, firstName, '', username, referralId);
        if (user) {
            if (referralId) {
                try {
                    ...
                    await safeSendMessage(referralId, `🎉 You earned ${REFERRAL_BONUS} Birr bonus! Someone registered using your referral link.`);
                } catch (e) {}
            }
            ...
            await safeSendMessage(chatId, '✅ Registration complete!', { reply_markup: { remove_keyboard: true } });
            await safeSendMessage(chatId, welcome, { parse_mode: 'HTML', ...mainMenu(userId) });
            await safeSendMessage(ADMIN_TELEGRAM_ID, `🆕 <b>New User Registered!</b>\n\n` +
                `👤 Name: ${firstName}\n🆔 ID: ${userId}\n📱 Username: @${username}`);
        }
    } catch (error) {
...
bot.onText(/\/play/, async (msg) => {
    ...
    try {
        ...
        await safeSendMessage(chatId, '❌ Please register first using /start.');
        return;
    }
    const stakeOptions = {
        reply_markup: {
...
bot.onText(/\/balance/, async (msg) => {
    ...
    const userId = msg.from.id;
    const balance = await getUserBalance(userId);
    await safeSendMessage(chatId, `💰 <b>Your Balance</b>\n\n${formatCurrency(balance)}`, { parse_mode: 'HTML' });
});

bot.onText(/\/deposit/, async (msg) => {
    const chatId = msg.chat.id;
    await safeSendMessage(chatId, '🏦 <b>Make a Deposit</b>\n\nPlease enter the amount you wish to deposit (minimum 50 ETB).', { parse_mode: 'HTML' });
    bot.once('text', async (msg) => {
        const amount = parseInt(msg.text);
        if (isNaN(amount) || amount < 50 || amount > 5000) {
            await safeSendMessage(chatId, '❌ Invalid amount. Please enter a number between 50 and 5000.');
            return;
        }
        ...
        await safeSendMessage(chatId, '💰 <b>Select payment method:</b>', { parse_mode: 'HTML', ...methodKeyboard });
    });
});
...
bot.onText(/\/withdraw/, async (msg) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    await safeSendMessage(chatId, '📤 <b>Withdrawal</b>\n\nPlease enter the amount you wish to withdraw.', { parse_mode: 'HTML' });
    bot.once('text', async (msg) => {
        const amount = parseInt(msg.text);
        if (isNaN(amount) || amount <= 0) {
            await safeSendMessage(chatId, '❌ Invalid amount.');
            return;
        }
        const balance = await getUserBalance(userId);
        if (amount > balance) {
            await safeSendMessage(chatId, `❌ Insufficient balance. Your balance is ${formatCurrency(balance)}.`);
            return;
        }
        const methodKeyboard = {
            reply_markup: {
        ...
        await safeSendMessage(chatId, '📤 <b>Select withdrawal method:</b>', { parse_mode: 'HTML', ...methodKeyboard });
    });
});
    ...
    bot.once('text', async (msg) => {
        ...
        if (recipientInput.startsWith('@')) {
            try {
                ...
                await safeSendMessage(chatId, '❌ User not found. Please make sure they are registered.');
                return;
            }
        } else {
            recipientId = parseInt(recipientInput);
            if (isNaN(recipientId)) {
                await safeSendMessage(chatId, '❌ Invalid input. Please enter a valid Telegram ID or @username.');
                return;
            }
        }
        try {
            ...
            await safeSendMessage(chatId, `❌ User with ID ${recipientId} is not registered.\n\n📩 <b>Invite them to join M-BINGO!</b>\nShare this link: https://t.me/M_bingo_bot?start=ref_${userId}`);
            return;
        }
        await safeSendMessage(chatId, '💵 <b>Enter amount to transfer:</b>', { parse_mode: 'HTML' });
        bot.once('text', async (msg2) => {
            ...
            try {
                ...
                await safeSendMessage(chatId, `✅ Successfully transferred ${formatCurrency(amount)} to user ${recipientId}.`);
            } catch (error) {
                await safeSendMessage(chatId, '❌ Transfer failed. Please try again later.');
            }
        });
    });
});

// (Continue with /bonus, /history, /profile, /help - using safeSendMessage)
bot.onText(/\/bonus/, async (msg) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    const inviteLink = `https://t.me/M_bingo_bot?start=ref_${userId}`;
    await safeSendMessage(chatId, `🎁 <b>Invite Friends & Earn!</b>\n\nShare your unique referral link with friends:\n\n${inviteLink}\n\nFor each friend who registers using your link, you earn <b>${REFERRAL_BONUS} Birr</b> bonus!`, { parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: "📤 Share Link", url: `https://t.me/share/url?url=${encodeURIComponent(inviteLink)}` }]] } });
});
...
bot.onText(/\/history/, async (msg) => {
    ...
    try {
        ...
        if (txs.length === 0) {
            await safeSendMessage(chatId, '📜 No transactions found.');
            return;
        }
        let msgText = '📜 <b>Your Transaction History</b>\n\n';
        txs.slice(0, 10).forEach(t => {
            const type = t.type === 'deposit' ? '💰 Deposit' : t.type === 'withdraw' ? '📤 Withdraw' : t.type === 'transfer' ? '🔄 Transfer' : '🎮 Game';
            const sign = t.type === 'deposit' ? '+' : '-';
            msgText += `${type}: ${sign}${formatCurrency(t.amount)} - ${t.status}\n`;
        });
        await safeSendMessage(chatId, msgText, { parse_mode: 'HTML' });
    } catch (e) {
        await safeSendMessage(chatId, '⚠️ Could not fetch transactions.');
    }
});
...
bot.onText(/\/profile/, async (msg) => {
    ...
    try {
        ...
        await safeSendMessage(chatId, `👤 <b>Your Profile</b>${adminBadge}\n\n📛 <b>Name:</b> ${escapeHtml(user.first_name || 'Player')}\n🆔 <b>ID:</b> ${user.telegramId}\n💰 <b>Balance:</b> ${formatCurrency(user.balance || 0)}\n🏆 <b>Games Played:</b> ${user.gamesPlayed || 0}\n🎖️ <b>Wins:</b> ${user.wins || 0}`, { parse_mode: 'HTML' });
    } catch (e) {
        await safeSendMessage(chatId, '⚠️ Could not fetch profile.');
    }
});

bot.onText(/\/help/, async (msg) => {
    const chatId = msg.chat.id;
    await safeSendMessage(chatId, `📖 <b>M-BINGO Help</b>\n\n/start - Start bot and register\n/play - Start a game\n/balance - Check your balance\n/deposit - Make a deposit\n/withdraw - Withdraw funds\n/transfer - Transfer funds\n/bonus - Invite friends and earn bonus\n/history - View history\n/profile - View profile\n\n<b>How to Play:</b>\n1. Tap "Play Game"\n2. Choose your stake\n3. Select cards in the web app\n4. Wait for 2+ players\n5. Game starts automatically\n6. First to complete a pattern wins 70% of the pool!`, { parse_mode: 'HTML' });
});
...
bot.on('callback_query', async (call) => {
    ...
    if (data.startsWith('stake_')) {
        ...
        await safeSendMessage(chatId, `✅ Stake set to ${stake} Birr.\n\n🎮 Click below to open the game and select your cards:`, { reply_markup: { inline_keyboard: [[{ text: "🎮 Open Game", web_app: { url: gameUrl } }], [{ text: "🔙 Back to Menu", callback_data: 'back_to_menu' }]] } });
        return;
    }
    ...
    switch (data) {
        ...
        case 'support': await safeSendMessage(chatId, '📞 <b>Contact Support</b>\n\n👤 @frezerabiy', { parse_mode: 'HTML' }); break;
        case 'patterns': await safeSendMessage(chatId, `🏆 <b>Winning Patterns</b>\n\n✅ Row - 5 numbers in a horizontal line\n✅ Column - 5 numbers in a vertical line\n✅ Diagonal - 5 numbers diagonally\n✅ Corners - All 4 corners\n✅ Full House - All numbers on card`, { parse_mode: 'HTML' }); break;
        case 'copy_invite': {
            const link = `https://t.me/M_bingo_bot?start=ref_${userId}`;
            await safeSendMessage(chatId, `📋 <b>Your Invite Link:</b>\n\n${link}`, { parse_mode: 'HTML' });
            break;
        }
        case 'back_to_menu': await safeSendMessage(chatId, '🎯 <b>Welcome back!</b>', { parse_mode: 'HTML', ...mainMenu(userId) }); break;
        case 'cancel': await safeSendMessage(chatId, '❌ Action cancelled.'); break;

        // ADMIN ACTIONS
        case 'admin_panel': {
            if (userId.toString() !== ADMIN_TELEGRAM_ID) break;
            await safeSendMessage(chatId, '👑 <b>Admin Panel</b>', { parse_mode: 'HTML', reply_markup: { inline_keyboard: [
        ...
        case 'admin_players': {
            ...
            try {
                ...
                await safeSendMessage(chatId, msg, { parse_mode: 'HTML' });
            } catch (error) { await safeSendMessage(chatId, '⚠️ Error fetching players.'); }
            break;
        }
        case 'admin_stats': {
            if (userId.toString() !== ADMIN_TELEGRAM_ID) break;
        ...
        case 'admin_deposits': {
            ...
            try {
                ...
                if (!deposits || deposits.length === 0) { await safeSendMessage(chatId, 'No pending deposits.'); break; }
                let msg = '📥 <b>Pending Deposits</b>\n\n';
                deposits.forEach(d => msg += `👤 ${escapeHtml(d.userName)} (${d.userId})\n💰 ${formatCurrency(d.amount)}\n📱 ${d.method}\n🆔 ${d.reference}\n\n`);
                await safeSendMessage(chatId, msg, { parse_mode: 'HTML' });
            } catch (error) { await safeSendMessage(chatId, '⚠️ Error fetching deposits.'); }
            break;
        }
        case 'admin_withdrawals': {
            ...
            try {
                ...
                if (!withdrawals || withdrawals.length === 0) { await safeSendMessage(chatId, 'No pending withdrawals.'); break; }
                let msg = '📤 <b>Pending Withdrawals</b>\n\n';
                withdrawals.forEach(w => msg += `👤 ${escapeHtml(w.userName)} (${w.userId})\n💰 ${formatCurrency(w.amount)}\n📱 ${w.method}\n🏦 ${w.destination}\n\n`);
                await safeSendMessage(chatId, msg, { parse_mode: 'HTML' });
            } catch (error) { await safeSendMessage(chatId, '⚠️ Error fetching withdrawals.'); }
            break;
        }
        default: break;
    }
...
bot.onText(/\/addbalance (\d+) (\d+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    if (userId.toString() !== ADMIN_TELEGRAM_ID) return safeSendMessage(chatId, '⛔ Unauthorized.');
    const targetId = parseInt(match[1]);
    const amount = parseInt(match[2]);
    try {
        const adminId = ADMIN_DB_ID || ADMIN_TELEGRAM_ID;
        await axios.post(`${API_URL}/api/admin/balance/add`, { userId: targetId, amount, adminId: adminId });
        await safeSendMessage(chatId, `✅ Added ${amount} Birr to player!`);
    } catch (e) { await safeSendMessage(chatId, '⚠️ Error adding balance.'); }
});
...
bot.onText(/\/broadcast (.+)/, async (msg, match) => {
    ...
    const userId = msg.from.id;
    if (userId.toString() !== ADMIN_TELEGRAM_ID) return safeSendMessage(chatId, '⛔ Unauthorized.');
    await safeSendMessage(chatId, `📢 Broadcast sent: "${match[1]}"`);
});

// ============================================================
// NOTIFICATION POLLER (Fully hardened)
// ============================================================
setInterval(async () => {
    try {
        ...
        for (let d of deposits.rows) {
            ...
            await safeSendMessage(d.telegram_id, msg, { parse_mode: 'HTML' });
        ...
        for (let w of withdrawals.rows) {
            ...
            await safeSendMessage(w.telegram_id, msg, { parse_mode: 'HTML' });
