...
// ---- All slash commands ----
// ** UPDATED: /play now opens the mini app in 'play' mode to ask for stake **
bot.onText(/\/play/, async (msg) => {
    ...
    const gameUrl = `${GAME_URL}?mode=play&userId=${userId}`;
    await bot.sendMessage(chatId, '🎮 <b>Select your stake inside the game:</b>\n\nClick below to open the game and choose your bet.', {
        parse_mode: 'HTML',
        reply_markup: {
            inline_keyboard: [
                [{ text: "🎮 Open M-Bingo", web_app: { url: gameUrl } }],
...
// ** UPDATED: Deposit asks amount, then method (no Telebirr), then opens Mini App **
...
// ** UPDATED: Withdraw asks amount, then address, then shows processing **
bot.onText(/\/withdraw/, async (msg) => {
    ...
    bot.once('text', async (msg) => {
        ...
        bot.once('text', async (msg2) => {
            const address = msg2.text.trim();
            if (!address) return;
            
            // Send request to backend (method defaults to 'bank' as we only ask address)
            try {
                ...
                if (response.data.success) {
                    await bot.sendMessage(chatId, '⏳ <b>Processing your withdrawal request...</b>\n\nPlease wait for admin approval. You will be notified here.', { parse_mode: 'HTML' });
                } else {
                    await bot.sendMessage(chatId, `❌ ${response.data.message || 'Request failed.'}`);
                }
            } catch (error) {
                await bot.sendMessage(chatId, '❌ Could not connect to server. Please try again.');
            }
        });
    });
});
...
6. First to complete a pattern wins 80% of the pool!
...
bot.on('callback_query', async (call) => {
    ...
    // ** NEW: Handle deposit method selection → open mini app **
    if (data.startsWith('deposit_method_')) {
        const parts = data.split('_');
        const method = parts[2]; // bank, cbe, ebirr
        ...
        await bot.sendMessage(chatId, `✅ You selected <b>${method.toUpperCase()}</b> for ${amount} Birr.\n\nClick below to complete your deposit.`, {
            parse_mode: 'HTML',
            reply_markup: {
                inline_keyboard: [[{ text: "💰 Open Deposit Page", web_app: { url: gameUrl } }]]
            }
        });
        return;
    }

    // Since the miniapp now only has 10 Birr, block any other stake callbacks
    if (data.startsWith('stake_')) {
        const stake = parseInt(data.split('_')[1]);
        if (stake !== 10) {
            await bot.sendMessage(chatId, '❌ Invalid stake amount. Only 10 Birr is allowed.');
            return;
        }
