...
async function chargeStake(client, userId, roomId, stake) {
    ...
    if (balance - reserved < stake) {
        throw new Error(`Insufficient available balance. You need ${stake} Birr.`);
    }
...
async function cancelRoomAndRefund(roomId, reason = 'Game cancelled') {
    ...
    try {
        ...
        for (const p of players.rows) {
            await releaseStake(client, p.user_id, roomId, true, Number(room.rows[0].stake));
        }
...
async function finishRoomWithWinners(roomId, winners, calledNumbers) {
    ...
    try {
        ...
        for (const p of allPlayers.rows) {
            await releaseStake(client, p.user_id, roomId, false, Number(room.stake));
        }
