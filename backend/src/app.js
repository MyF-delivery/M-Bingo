...
app.post('/api/transfer', async (req, res) => {
    ...
    try {
        ...
        const fromUser = await client.query(
            `SELECT id, balance, COALESCE(withdrawal_reserved,0) AS withdrawal_reserved FROM users WHERE id=$1 FOR UPDATE`,
            [fromId]
        );
        if (!fromUser.rows.length) throw new Error('Sender not found');
        ...
        if (!toUser.rows.length) throw new Error('Recipient not found');

        const fromBalance = Number(fromUser.rows[0].balance);
        const reserved = Number(fromUser.rows[0].withdrawal_reserved || 0);
        ...
        const toBalance = Number(toUser.rows[0].balance);

        // Update balances
        await client.query(`UPDATE users SET balance = balance - $1 WHERE id = $2`, [amountNum, fromId]);
        await client.query(`UPDATE users SET balance = balance + $1 WHERE id = $2`, [amountNum, toId]);

        // Record ledger entries
        await addLedger(client, fromId, 'TRANSFER_OUT', amountNum, fromBalance, fromBalance - amountNum, 'TRANSFER', toId);
        await addLedger(client, toId, 'TRANSFER_IN', amountNum, toBalance, toBalance + amountNum, 'TRANSFER', fromId);
...
app.get('/api/games/:userId', async (req, res) => {
    try {
        const userId = req.params.userId;
        const result = await pool.query(
            `SELECT * FROM game_history WHERE winner_id = $1 OR (SELECT user_id FROM room_players WHERE room_id = game_history.room_id AND user_id = $1 LIMIT 1) IS NOT NULL
             ORDER BY created_at DESC LIMIT 50`,
            [userId]
        );
        // Alternative: simpler query if we store player IDs in game_history
        // We'll keep a separate query: get rooms where the user played.
        // Better: use room_players to find rooms, then join game_history.
        // We'll do a more efficient query:
    ...
    try {
        const result = await pool.query(
            `INSERT INTO withdrawals (user_id, amount, method, destination, status)
             VALUES ($1, $2, $3, $4, 'PENDING')
             RETURNING *`,
            [user.id, value, method, account]
        );
        // Reserve funds
        await pool.query(`UPDATE users SET withdrawal_reserved = COALESCE(withdrawal_reserved,0) + $1 WHERE id = $2`, [value, user.id]);
        res.json({ success: true, withdrawal: result.rows[0] });
    } catch (e) {
        res.status(500).json({ success: false, message: e.message });
    }
...
app.post('/api/referral/process', async (req, res) => {
    ...
    try {
        ...
        // Check that both users exist and referral not already processed
        const existing = await client.query(
        ...
        if (!referrer.rows.length) throw new Error('Referrer not found');

        const before = Number(referrer.rows[0].balance);
        const after = before + REFERRAL_BONUS;

        await client.query(`UPDATE users SET balance = $1 WHERE id = $2`, [after, referrerId]);
        await addLedger(client, referrerId, 'REFERRAL_BONUS', REFERRAL_BONUS, before, after, 'REFERRAL', newUserId);

        // Record referral
        await client.query(
            `INSERT INTO referrals (referrer_id, referred_id, bonus_amount, status, paid_at)
             VALUES ($1, $2, $3, 'PAID', CURRENT_TIMESTAMP)`,
            [referrerId, newUserId, REFERRAL_BONUS]
        );
