...
app.post('/api/users/register', async (req, res) => {
    try {
        ...
        try {
            ...
            // CHANGED: 500 to 50 for signup bonus
            const userResult = await client.query(
                `INSERT INTO users
            ...
            res.json({
                success: true,
                user,
                message: 'User registered with 50 Birr bonus!'
            });
        } catch (e) {
            await client.query('ROLLBACK').catch(() => {});
            throw e;
...
// ============================================================
// NEW ENDPOINTS FOR TELEGRAM BOT / FRONTEND
// ============================================================

// Send notification to Admin (Helper Function)
...
// 5. Deposit request (for bot/frontend) - WITH ADMIN NOTIFICATION
...
// 6. Withdraw request (for bot/frontend) - WITH ADMIN NOTIFICATION
...
// 8. Admin action (generic deposit/withdraw/ban/unban via HTTP)
app.post('/api/admin/action', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        // Check amount for other actions
...
// NEW: Detailed User Info Endpoint
app.get('/api/admin/user/:userId', requireAdmin, async (req, res) => {
    try {
        const userResult = await pool.query(
            `SELECT id, telegram_id, first_name, username, balance, locked_balance, withdrawal_reserved,
                    total_wins, total_games_played, total_winnings, is_admin, is_banned, created_at, last_login
...
// NEW: Endpoint for the Frontend Admin Panel (Updated to include is_banned)
...
app.post('/api/admin/login', async (req, res) => {
    try {
        ...
        // Return the database UUID so the frontend can use it for /api/admin/data
...
// UPDATED: Include totalStakes in stats for Laser (30% cut)
    ...
    try {
        const result = await pool.query(`
            SELECT
              ...
              (SELECT COALESCE(SUM(amount),0) FROM deposits WHERE status='APPROVED')::numeric AS "totalStakes"
        `);
            ...
            switch (type) {
                case 'auth': {
                    ...
                    if (user.is_banned === true) {
                        return sendError(ws, 'You are banned from playing. Contact Admin.');
                    }
