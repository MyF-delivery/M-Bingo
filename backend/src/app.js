...
// 🟢 UPDATED: chargeStake now accepts a cardCount and deducts stake * cardCount
async function chargeStake(client, userId, roomId, stake, cardCount) {
    ...
    // Calculate total cost for this round
...
// 🟢 UPDATED: startGame now calculates card costs and charges players
async function startGame(roomId) {
    ...
    try {
        ...
        // 🔥 NEW LOGIC: Charge stake * cardCount for each player
        let totalStake = 0;
...
async function cancelRoomAndRefund(roomId, reason = 'Game cancelled') {
    ...
    try {
        ...
        // For cancellations, refund the full locked amount
...
async function finishRoomNoWinner(roomId) {
    ...
    try {
        ...
        // In a no-winner scenario, refund the stake.
        for (const p of players.rows) {
            await releaseStake(client, p.user_id, roomId, true);
        }
...
async function finishRoomWithWinners(roomId, winners, calledNumbers) {
    ...
    try {
        ...
        // Remove the remaining locked stake from every player (winners and losers).
                ...
                case 'startSelection': {
                    ...
                    try {
                        ...
                        if (!joined.rows.length) {
                            // 🟢 Note: We now delay the charge until cards are submitted in startGame()
                            await client.query(
