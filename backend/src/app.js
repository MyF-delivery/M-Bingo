...
async function chargeStake(client, userId, roomId, stake) {
    const userResult = await client.query(
        `SELECT id, balance, locked_balance FROM users WHERE id = $1 FOR UPDATE`,
        [userId]
    );
    ...
    if (balance < stake) {
        throw new Error(`Insufficient balance. You need ${stake} Birr.`);
    }

    const newBalance = balance - stake;
    const newLocked = locked + stake;
    ...
    await addLedger(client, userId, 'STAKE_LOCK', stake, balance, newBalance, 'ROOM', roomId);
}

async function releaseStake(client, userId, roomId, refund = false) {
    ...
    if (refund) {
        ...
        const after = before + locked;

        await client.query(
            `UPDATE users SET balance = $1, locked_balance = 0 WHERE id = $2`,
            [after, userId]
        );

        await addLedger(client, userId, 'STAKE_REFUND', locked, before, after, 'ROOM', roomId);
    } else {
        await client.query(
            `UPDATE users SET locked_balance = 0 WHERE id = $1`,
            [userId]
        );
    }
}
...
async function startGame(roomId) {
    ...
    try {
        ...
        const totalStake = Number(room.stake) * totalPlayers;
...
async function callNextNumber(roomId) {
    ...
    try {
        ...
        // Server-side winner check immediately after each call.
        const winners = [];
...
function startNumberCaller(roomId) {
    ...
    // Call the first number immediately so players don't wait 10 seconds.
...
async function finishRoomWithWinners(roomId, winners, calledNumbers) {
    ...
    try {
        ...
        // One player/card is enough to end the game. If several cards hit
        // on the same called number, unique winning players share the pool.
        const unique = [];
        ...
        // Remove the remaining locked stake from every player.
...
app.post('/api/users/register', async (req, res) => {
    try {
        ...
        if (!telegramCheck.valid && !browserTesting) {
...
app.post('/api/admin/balance/add', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        await client.query('BEGIN');
        const u = await client.query(`SELECT balance FROM users WHERE id=$1 FOR UPDATE`, [target.id]);
    ...
    ws.on('message', async raw => {
        try {
            ...
            switch (type) {
                ...
                case 'startSelection': {
                    ...
                    try {
                        ...
                        if (!joined.rows.length) {
                            await chargeStake(client, user.id, room.id, stake);
                ...
                case 'adminLogin': {
                    const candidate = data.userId || ws.userId;
                    const admin = await findUser(candidate);
...
async function transferBalance(fromId, toId, amount) {
    ...
    try {
        ...
        if (fromBefore < amount) throw new Error('Insufficient balance');
