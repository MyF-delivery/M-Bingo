...
// ---------- Health Checks ----------
app.get('/', (req, res) =>
  res.json({ ok: true, service: 'm-bingo', status: 'online', websocket: true, health: '/health', version: '3.4.0' })
);
...
// ---------- Authoritative remaining selection seconds ----------
function computeSelectionTimeLeft(snapshot) {
  if (!snapshot || snapshot.state !== 'SELECTING') return 0;
  const totalSec = Number(snapshot.countdown_seconds || config.BINGO_SELECTION_SECONDS || 60);
  const startedAt = new Date(snapshot.created_at).getTime();
  const elapsedSec = Math.max(0, (Date.now() - startedAt) / 1000);
  return Math.max(0, Math.ceil(totalSec - elapsedSec));
}

// ---------- WITHDRAWALS PAUSED CHECK ----------
async function isWithdrawalsPaused() {
  try {
    const r = await pool.query(`SELECT value FROM admin_settings WHERE key = 'withdrawals_paused' LIMIT 1`);
...
async function startGame(roomId) {
  ...
  try {
    ...
    // ⭐ Guard against double-start races (WS handler + auto-loop)
    if (room.state !== 'SELECTING' && room.state !== 'WAITING') {
      await client.query('ROLLBACK');
      console.log(`⏭ startGame skipped — room ${roomId} state=${room.state}`);
      return null;
    }

    const playersResult = await client.query(
      `SELECT rp.user_id, rp.cards, u.balance
    ...
    const allPlayers = playersResult.rows.filter(p => Array.isArray(p.cards) && p.cards.length > 0);

    // ⭐ Filter out players who can't afford their cards.
    //    We clear their cards and notify them, rather than failing the whole game.
    const players = [];
    const removed = [];
    for (const p of allPlayers) {
      ...
      if (payAmount > 0 && balance < payAmount) {
        removed.push({ user_id: p.user_id, cards: cards.length, need: payAmount, have: balance });
      } else {
        players.push({ user_id: p.user_id, cards, payAmount });
      }
    }

    if (removed.length) {
      console.log(`⚠️  room=${roomId} removing ${removed.length} broke player(s):`, JSON.stringify(removed));
      for (const r of removed) {
        await client.query(
          `UPDATE room_players SET cards = '[]'::jsonb WHERE room_id = $1 AND user_id = $2`,
          [roomId, r.user_id]
        );
      }
    }

    if (players.length < Number(room.min_players || 2)) {
      await client.query('ROLLBACK');
      console.log(`⏭ startGame skipped — only ${players.length} funded player(s) with cards (need ${room.min_players || 2})`);
      // Broadcast the removals so the removed users see what happened
      // (they'll also get the individual error message below in a follow-up.)
      for (const r of removed) {
        sendToUser(String(r.user_id), 'gameError', {
          message: `⚠️ ካርድዎ ተሰርዟል: ባላንስዎ በቂ አይደለም (${r.have} / ${r.need} ብር)። እባክዎ ይሙሉ እና እንደገና ይምረጡ።`
        });
      }
      return null;
    }
    ...
    console.log(`🎮 startGame: room=${roomId}, players=${players.length}, totalPool=${totalPool}, winnerPool=${winnerPool}`);

    // Notify removed players (outside the transaction, in case broadcast fails)
    for (const r of removed) {
      sendToUser(String(r.user_id), 'gameError', {
        message: `⚠️ ካርድዎ ተሰርዟል: ባላንስዎ በቂ አይደለም (${r.have} / ${r.need} ብር)። እባክዎ ይሙሉ እና እንደገና ይምረጡ።`
      });
    ...
    return { success: true, prizePool: winnerPool, totalPool, commission, players, removed };
  } catch (e) {
...
// ⭐ Start + broadcast in one place so the WS handler and the
//    auto-loop produce identical messages.
async function startGameAndBroadcast(roomId) {
  // Re-snapshot right before starting so the ready list is current
  const snap = await roomSnapshot(roomId);
  if (!snap) return null;
  const readyPlayers = (snap.players || []).filter(p => Array.isArray(p.cards) && p.cards.length);
  if (readyPlayers.length < Number(snap.min_players || 2)) return null;

  const result = await startGame(roomId);
  if (!result) return null;

  // ⭐ Re-snapshot AFTER startGame so we broadcast the correct
  //    card set (funded players only — broke players were removed).
  const fresh = await roomSnapshot(roomId);
  const cardCount = (fresh?.players || []).reduce((s, p) => s + (p.cards?.length || 0), 0);
  const cards = await getGameCards(roomId, []);
  broadcastToRoom(roomId, 'gameStart', {
    roomId,
    gameNumber: (fresh || snap).game_number,
    stake: Number((fresh || snap).stake),
  ...
  console.log(`📢 gameStart broadcast room=${roomId} cards=${cardCount} prizePool=${result.prizePool}`);
  return result;
}

// ============================================================
// endGame — winner balance credited correctly
// ============================================================
async function endGame(roomId, winners) {
  ...
  try {
    ...
    prizePerWinner = winners.length ? Number((prizePool / winners.length).toFixed(2)) : 0;

    console.log(`💰 endGame: pool=${prizePool}, winners=${winners.length}, prizePerWinner=${prizePerWinner}`);
  ...
  try {
    await pool.query(
      `INSERT INTO game_history (room_id, prize_pool, ended_at)
       SELECT id, prize_pool, CURRENT_TIMESTAMP FROM rooms WHERE id = $1`,
      [roomId]
    );
  } catch (err) {
    console.warn('game_history insert failed (non-fatal):', err.message);
  }
  ...
  const tick = async () => {
    try {
      ...
      broadcastToRoom(roomId, 'numberCalled', {
        number: num, calledNumbers: already, calledCount: already.length, cards,
      });
  ...
  try {
    ...
    if (snap) {
      broadcastToRoom(roomId, 'gameStateUpdate', {
        ...
        selectedCards: [],
        selectionTimeLeft: computeSelectionTimeLeft(snap),
...
// ⭐ Send a message to one specific user by their internal user id.
...
app.post('/api/withdraw/request', async (req, res) => {
  ...
  const user = await findUser(userId);
  if (!user) return res.status(404).json({ success: false, message: 'User not found' });
  if (await isWithdrawalsPaused()) {
    return res.status(503).json({ success: false, message: 'Withdrawals are temporarily paused. Please try again later.' });
  }
  const value = safeNumber(amount);
...
const wss = new WebSocket.Server({
  server,
  verifyClient: () => { console.log('🔌 WebSocket connection attempt'); return true; }
});
...
// ============================================================
// ⭐ AUTO-START LOOP — server owns the selection timer.
// ============================================================
const autoLoopLogAt = new Map();

function logAutoLoop(roomId, msg) {
  const now = Date.now();
  const last = autoLoopLogAt.get(roomId) || 0;
  if (now - last < 2000) return;
  autoLoopLogAt.set(roomId, now);
  console.log(`[auto-loop] room=${roomId} ${msg}`);
}

setInterval(async () => {
  try {
    const rooms = await pool.query(
      `SELECT id FROM rooms WHERE state = 'SELECTING'`
    );
    for (const r of rooms.rows) {
      const snap = await roomSnapshot(r.id);
      if (!snap || snap.state !== 'SELECTING') continue;
      const remaining = computeSelectionTimeLeft(snap);
      if (remaining > 0) continue;

      const readyCount = (snap.players || []).filter(p => Array.isArray(p.cards) && p.cards.length).length;
      const minPlayers = Number(snap.min_players || 2);

      logAutoLoop(r.id, `timer=0 readyCount=${readyCount} min=${minPlayers} stake=${snap.stake} game#=${snap.game_number}`);

      if (readyCount >= minPlayers) {
        try {
          const result = await startGameAndBroadcast(r.id);
          if (result) {
            console.log(`✅ [auto-loop] room=${r.id} game STARTED — prizePool=${result.prizePool} players=${result.players.length} removed=${result.removed.length}`);
            autoLoopLogAt.delete(r.id);
          }
        } catch (e) {
          console.error(`❌ [auto-loop] room=${r.id} startGame error:`, e.message);
        }
      } else {
        logAutoLoop(r.id, `not enough cards → reset created_at (${readyCount}/${minPlayers})`);
        await pool.query(`UPDATE rooms SET created_at = NOW() WHERE id = $1`, [r.id]);
        const fresh = await roomSnapshot(r.id);
        if (fresh) {
          broadcastToRoom(r.id, 'gameStateUpdate', {
            status: 'selecting', roomId: r.id, stake: Number(fresh.stake),
            gameNumber: fresh.game_number,
            selectedCards: fresh.players.flatMap(p => p.cards || []),
            selectionTimeLeft: computeSelectionTimeLeft(fresh),
            players: fresh.players,
          });
        }
      }
    }
  } catch (e) { console.error('Auto-start loop error:', e.message); }
}, 1000);
      ...
      switch (type) {
          ...
          send(ws, 'init', {
            ...
            gameState: snapshot
              ? {
                  ...
                  selectionTimeLeft: computeSelectionTimeLeft(snapshot),
        ...
        case 'startSelection': {
          ...
          try {
            ...
            const snapNow = await roomSnapshot(room.id);
            if (computeSelectionTimeLeft(snapNow) <= 1) {
              await client.query(`UPDATE rooms SET created_at = NOW() WHERE id = $1`, [room.id]);
            }
        ...
        case 'resetSelection': {
          ...
          try {
            await client.query('BEGIN');
            const roomRes = await client.query(`SELECT * FROM rooms WHERE id = $1 FOR UPDATE`, [ws.roomId]);
            if (!roomRes.rows.length) throw new Error('Room not found');
            const room = roomRes.rows[0];
            if (room.state === 'PLAYING') { await client.query('ROLLBACK'); break; }
            await client.query(
              `UPDATE room_players SET cards = '[]'::jsonb, is_winner = FALSE, winning_amount = 0 WHERE room_id = $1`,
              [ws.roomId]
            );
            await client.query(
              `UPDATE rooms SET state = 'SELECTING', status = 'SELECTING',
                   called_numbers = '[]'::jsonb, countdown_seconds = $1, created_at = NOW()
               WHERE id = $2`,
        ...
        case 'startGame': {
          ...
          try { await startGameAndBroadcast(ws.roomId); }
        ...
        case 'withdrawRequest': {
          ...
          const user = await findUser(userId || ws.userId);
          if (!user) return sendError(ws, 'User not found');
          if (await isWithdrawalsPaused()) return sendError(ws, 'Withdrawals are temporarily paused. Please try again later.');
          const value = safeNumber(amount);
...
process.on('SIGTERM', async () => {
  for (const [roomId, timer] of roomTimers.entries()) { clearTimeout(timer); roomTimers.delete(roomId); }
  server.close(async () => { await pool.end(); process.exit(0); });
});

module.exports = { app, server, http, wss, pool };