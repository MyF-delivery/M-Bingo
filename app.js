...
const axios = require('axios'); // kept for possible future use
...
// ============================================================
// VARIFY.ET INTEGRATION (kept but bypassed)
// ============================================================
...
const VARIFY_WEBHOOK_SECRET = process.env.VARIFY_WEBHOOK_SECRET;
...
// ============================================================
// GAME LOGIC (commission tracking integrated)
// ============================================================

// --- startGame: deduct stakes, compute commission, set prize pool ---
async function startGame(roomId) {
  ...
  try {
    ...
    // 6. Start the number caller (asynchronous)
...
// --- endGame: distribute prize, update room state, record commission ---
async function endGame(roomId, winners) {
  ...
  try {
    ...
    // 4. Insert game history with commission
    await client.query(
...
// --- Number caller (simplified stub) ---
const roomTimers = new Map();

function startNumberCaller(roomId) {
  // In production, you'd implement real number generation and BINGO detection.
  // This stub waits 8 seconds and then ends the game with a dummy winner for demonstration.
  if (roomTimers.has(roomId)) return;
  console.log(`🔢 Starting number caller for room ${roomId}`);
  const timer = setTimeout(async () => {
    try {
      // Simulate a winner (replace with actual logic)
      const dummyWinners = [
        { user_id: 1, card_number: 5, patterns: ['ROW', 'COLUMN'] }
      ];
      await endGame(roomId, dummyWinners);
      // Broadcast win to room (via WebSocket)
      broadcastToRoom(roomId, 'gameEnd', { winners: dummyWinners, prizePerWinner: 10 });
    } catch (err) {
      console.error('Number caller error:', err);
    } finally {
...
// --- Helper: room snapshot (simplified) ---
async function roomSnapshot(roomId) {
  const result = await pool.query(
    `SELECT r.*, 
            COALESCE(jsonb_agg(jsonb_build_object('user_id', rp.user_id, 'cards', rp.cards)) FILTER (WHERE rp.user_id IS NOT NULL), '[]') AS players
     FROM rooms r
...
// --- Helper: get game cards (simplified) ---
async function getGameCards(roomId, calledNumbers) {
  // In real implementation, fetch cards for players in this room
  return [];
}
...
// ---------- API Routes (deposit/withdraw, admin, etc.) ----------
...
// ============================================================
// DEPOSIT via MANUAL (bypass varify.et)
// ============================================================
...
// ============================================================
// WITHDRAWAL via MANUAL (bypass varify.et)
// ============================================================
app.post('/api/withdraw/request', async (req, res) => {
  ...
  if (!account) return res.status(400).json({ success: false, message: 'Account (phone) is required' });
...
app.post('/webhook/varify/deposit', async (req, res) => {
  ...
  try {
    ...
    if (deposit.varify_status === 'MANUAL') {
      return res.json({ received: true, note: 'manual deposit, ignored' });
    }
...
app.post('/webhook/varify/withdraw', async (req, res) => {
  const { reference, transaction_id, status, amount, method, phone } = req.body;
  console.log('Withdrawal webhook received (ignored unless manual):', req.body);
  try {
    ...
    if (withdrawal.varify_status === 'MANUAL') {
      return res.json({ received: true, note: 'manual withdrawal, ignored' });
    }
...
// Override the broadcast helper to use the local one
broadcastToRoom = broadcastToRoom; // already defined

// ---------- Lobby Data Broadcast ----------
      ...
      switch (type) {
        case 'auth': {
          ...
          send(ws, 'init', {
            ...
            balance: Number(user.balance),          // ← ADDED LINE
        ...
        case 'startSelection': {
          ...
          try {
            await client.query('BEGIN');
            // find or create room (simplified – you may have a separate function)
            const roomResult = await client.query(
            ...
            if (roomResult.rows.length) {
              ...
              const newRoom = await client.query(
                `INSERT INTO rooms (stake, state, status, countdown_seconds, created_at)
                 VALUES ($1, 'SELECTING', 'SELECTING', $2, CURRENT_TIMESTAMP)
                 RETURNING *`,
            ...
            ws.roomId = room.id;
            clients.set(ws.userId, ws);
            // schedule selection timeout (stub)
        ...
        case 'getGameState': {
          ...
          if (snapshot) {
            const selectedCards = snapshot.players.flatMap((p) => p.cards || []);
            const cards = snapshot.state === 'PLAYING' ? await getGameCards(ws.roomId, snapshot.called_numbers || []) : [];
        ...
        case 'depositRequest': {
          ...
          try {
            ...
            await notification.sendAdminNotification(
              `💰 Deposit request (manual) via WS\nUser: ${user.first_name} (${user.telegram_id})\nAmount: ${value} Birr\nMethod: ${method}\nPhone: ${phone}\nReference: ${manualRef}`
            );
          } catch (err) {
            sendError(ws, err.message || 'Deposit creation failed');
          }
          break;
        }
        ...
        case 'withdrawRequest': {
          ...
          try {
            ...
            await notification.sendAdminNotification(
              `🏦 Withdrawal request (manual) via WS\nUser: ${user.first_name} (${user.telegram_id})\nAmount: ${value} Birr\nMethod: ${method}\nAccount: ${account}\nReference: ${withdrawal.varify_reference}`
            );
          } catch (err) {
            sendError(ws, err.message || 'Withdrawal creation failed');
          }
          break;
        }
...
server.listen(config.PORT, async () => {
  console.log('============================================================');
  console.log('🎯 M-BINGO MODULAR SERVER (manual deposit/withdrawal + commission)');
