...
async function startGame(roomId) {
  ...
  try {
    ...
    // Start the number caller AFTER commit
...
// ============================================================
// NUMBER CALLER – real numbers + real BINGO detection + room reset
// ============================================================
...
function startNumberCaller(roomId) {
  ...
  const tick = async () => {
    try {
      ...
      // All 75 numbers called → end with no winner
      ...
      await pool.query(
        ...
        [JSON.stringify(already), roomId]
      );

      // Fetch updated cards with marked cells
      const cards = await getGameCards(roomId, already);

      // Broadcast the called number to everyone in the room
      ...
      if (winners.length > 0) {
        ...
        // Reset the room after 10 seconds for the next game
...
// Reset room to a fresh SELECTING state so a new game can start
    ...
    try {
      ...
      switch (type) {
        case 'auth': {
          ...
          // If the room is PLAYING, send a proper gameStart with cards + called numbers
        ...
        case 'startSelection': {
          ...
          try {
            ...
            // ─────────────────────────────────────────────────────────
            // If a game is already PLAYING at this stake, join as a
            // spectator (0 cards) and send the current game state.
            // ─────────────────────────────────────────────────────────
            const playingRoom = await client.query(
              `SELECT * FROM rooms WHERE stake = $1 AND state = 'PLAYING' ORDER BY created_at DESC LIMIT 1`,
              [stake]
            );
        ...
        case 'startGame': {
          ...
          try {
            ...
            if (result) {
              // Build the gameStart payload from the DB (with cards)
              const calledNumbers = [];
              const cards = await getGameCards(ws.roomId, calledNumbers);
          ...
          if (snapshot) {
            ...
            send(ws, 'gameStateUpdate', {
              ...
              selectionTimeLeft: snapshot.state === 'SELECTING'
                ? Math.max(0, Math.ceil((new Date(snapshot.created_at).getTime() + Number(snapshot.countdown_seconds || config.BINGO_SELECTION_SECONDS) * 1000 - Date.now()) / 1000))
                : 0,
              players: snapshot.players,
              cards,
            });
