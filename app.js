...
// notification is still imported in case other code paths use it,
// but the deposit/withdraw notification calls have been removed so
// only bot.js (with inline ✅/❌ buttons) notifies the admin.
...
// ---------- Health Checks ----------
app.get('/', (req, res) =>
  res.json({ ok: true, service: 'm-bingo', status: 'online', websocket: true, health: '/health', version: '3.2.2' })
);
...
// ⭐ Compute authoritative remaining selection seconds from a room snapshot.
//    Uses created_at + countdown_seconds so late joiners and reconnecting
//    clients always get the REAL remaining time, not the full countdown.
...
// The AI agent writes 'withdrawals_paused' = 'true' into admin_settings.
// This helper is used by both the REST and WebSocket withdraw handlers.
// Safe to call even if the table doesn't exist yet (returns false).
async function isWithdrawalsPaused() {
  try {
    const r = await pool.query(
      `SELECT value FROM admin_settings WHERE key = 'withdrawals_paused' LIMIT 1`
    );
    return String(r.rows[0]?.value || '').toLowerCase() === 'true';
  } catch (e) {
    // Table missing or DB error → treat as not paused (fail open).
    return false;
  }
}
...
async function endGame(roomId, winners) {
  ...
  try {
    ...
    for (const w of winners) {
      ...
      if (!userBefore.rows.length) {
        console.warn(`⚠️ Winner user not found: ${w.user_id}`);
        continue;
      }
      ...
      console.log(`✅ Winner ${w.user_id} credited ${prizePerWinner} (new balance: ${after})`);
    }
    ...
    await client.query('COMMIT');
    console.log(`✅ endGame committed for room ${roomId}`);
  } catch (e) {
  ...
  // Insert game_history OUTSIDE the transaction (non-critical).
  try {
    await pool.query(
...
app.post('/api/deposit/request', async (req, res) => {
  ...
  try {
    ...
    // ⭐ Notification is now handled exclusively by bot.js cron
    //    (with ✅/❌ inline buttons). Removing this call prevents
    //    duplicate admin messages per deposit request.
...
app.post('/api/withdraw/request', async (req, res) => {
  ...
  // Block withdrawals when paused by AI agent auto-pause
  ...
  try {
    ...
    await client.query('COMMIT');
    // ⭐ Notification handled by bot.js cron — see above.
    ...
    try {
      ...
      switch (type) {
        case 'auth': {
          ...
          // ⭐ Compute real remaining selection seconds so a reconnect
          //    mid-selection doesn't show "0" and immediately submit.
          const initSelectionTimeLeft = computeSelectionTimeLeft(snapshot);
          send(ws, 'init', {
            ...
            gameState: snapshot
              ? {
                  ...
                  selectionTimeLeft: initSelectionTimeLeft,
        ...
        case 'resetSelection': {
          ...
          try {
            ...
            const snapshot = await roomSnapshot(ws.roomId);
            if (snapshot) {
              // ⭐ Real remaining seconds from the just-updated created_at
              broadcastToRoom(ws.roomId, 'gameStateUpdate', {
                status: 'selecting',
                roomId: ws.roomId,
                stake: Number(snapshot.stake),
        ...
        case 'depositRequest': {
          ...
          try {
            ...
            // ⭐ Notification handled by bot.js cron only (see comments above).
        ...
        case 'withdrawRequest': {
          ...
          // Block withdrawals when paused by AI agent auto-pause
          if (await isWithdrawalsPaused()) {
            return sendError(ws, 'Withdrawals are temporarily paused. Please try again later.');
          }
