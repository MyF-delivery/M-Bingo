...
// ---------- Health Checks ----------
app.get('/', (req, res) =>
  res.json({ ok: true, service: 'm-bingo', status: 'online', websocket: true, health: '/health', version: '3.2.1' })
);
        ...
        case 'startSelection': {
          ...
          try {
            ...
            if (snapshot) {
              ...
              // Compute authoritative remaining time so late joiners get it correctly
              const elapsedSec = Math.max(0, Math.floor((Date.now() - new Date(snapshot.created_at).getTime()) / 1000));
              ...
              broadcastToRoom(room.id, 'gameStateUpdate', {
                ...
                selectionTimeLeft: remainingSec,
