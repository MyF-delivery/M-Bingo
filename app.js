...
// ---------- Health Checks ----------
app.get('/', (req, res) =>
  res.json({ ok: true, service: 'm-bingo', status: 'online', websocket: true, health: '/health', version: '3.3.1' })
);
...
async function startGame(roomId) {
  ...
  try {
    ...
    const players = playersResult.rows.filter(p => Array.isArray(p.cards) && p.cards.length > 0);
    if (players.length < Number(room.min_players || 2)) {
      await client.query('ROLLBACK');
      console.log(`⏭ startGame skipped — only ${players.length} player(s) with cards`);
      return null;
    }
    ...
    console.log(`🎮 startGame: room=${roomId}, totalPool=${totalPool}, winnerPool=${winnerPool}`);
...
//    Logs throttled per room so Render logs don't spam.
// ============================================================
const autoLoopLogAt = new Map(); // roomId -> last log timestamp

function logAutoLoop(roomId, msg) {
  const now = Date.now();
  const last = autoLoopLogAt.get(roomId) || 0;
  if (now - last < 2000) return; // throttle 2s per room
    ...
    for (const r of rooms.rows) {
      ...
      if (readyCount >= minPlayers) {
        try {
          ...
          if (result) {
            console.log(`✅ [auto-loop] room=${r.id} game STARTED — prizePool=${result.prizePool}`);
            autoLoopLogAt.delete(r.id);
          } else {
            logAutoLoop(r.id, `startGameAndBroadcast returned null (guard rejected)`);
          }
        } catch (e) {
          console.error(`❌ [auto-loop] room=${r.id} startGame error:`, e.message);
          console.error(`   stack:`, e.stack);
          // Throttled repeat of the error message so it's visible
          logAutoLoop(r.id, `startGame threw: ${e.message}`);
        }
      } else {
