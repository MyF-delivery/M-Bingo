...
// ---------- Health Checks ----------
app.get('/', (req, res) =>
  res.json({ ok: true, service: 'm-bingo', status: 'online', websocket: true, health: '/health', version: '3.3.0' })
);
...
//    Every second, for each SELECTING room:
//      • If timer expired and 2+ ready players → auto start game
//      • If timer expired and <2 ready players  → reset created_at
//        so the next player sees a fresh 60 s window
//    Client no longer needs to send startGame/resetSelection.
// ============================================================
setInterval(async () => {
  try {
    ...
    for (const r of rooms.rows) {
      ...
      if (readyCount >= Number(snap.min_players || 2)) {
        try { await startGameAndBroadcast(r.id); }
        catch (e) { console.error('Auto-start error:', e.message); }
      } else {
        // Not enough cards selected — give a fresh 60 s window
      ...
      switch (type) {
        ...
        case 'startSelection': {
          ...
          try {
            ...
            // Case 1: a game is currently PLAYING at this stake →
            // join as spectator, send gameStart with all cards.
            ...
            // Case 2: SELECTING / WAITING room at this stake → join it.
            // We do NOT touch created_at here, so the timer keeps its
            // real remaining time. Only if the timer already expired do
            // we reset it (so the new player sees a fresh 60 s window).
            ...
            // If the pre-existing timer already expired, give a fresh window.
