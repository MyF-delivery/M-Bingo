...
// ============================================================
// LOBBY DATA BROADCAST (NEW)
// ============================================================
async function broadcastLobbyData() {
  try {
    // Query rooms grouped by stake
    const result = await pool.query(`
      SELECT 
        r.stake,
        COALESCE(SUM(jsonb_array_length(rp.cards)), 0)::int AS selected_cards_count,
        COUNT(DISTINCT rp.user_id)::int AS players_count,
        CASE WHEN EXISTS (
          SELECT 1 FROM rooms r2 
          WHERE r2.stake = r.stake AND r2.state = 'PLAYING'
        ) THEN 'playing' ELSE 'waiting' END AS status
      FROM rooms r
      LEFT JOIN room_players rp ON rp.room_id = r.id AND rp.left_at IS NULL
      WHERE r.state <> 'ENDED'
      GROUP BY r.stake
    ...
    const data = rows.map(row => ({
      ...
      selectedCardsCount: Number(row.selected_cards_count),
      playersCount: Number(row.players_count),
      status: row.status
    }));
    // Ensure all stakes (10,20,50) are present
    const allStakes = [10, 20, 50];
    const merged = allStakes.map(stake => {
      const found = data.find(d => d.stake === stake);
      return found || { stake, selectedCardsCount: 0, playersCount: 0, status: 'waiting' };
    });

    const payload = { type: 'lobbyData', data: merged };
    for (const ws of clients.values()) {
...
// Broadcast lobby data every 5 seconds
  ...
  ws.on('message', async (raw) => {
    try {
      ...
      switch (type) {
        ...
        // NEW: Handle lobby data request
        case 'getLobbyData': {
          await broadcastLobbyData(); // send immediately to the requesting client
          break;
        }
