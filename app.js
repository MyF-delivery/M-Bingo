...
// ============================================================
// VARIFY.ET INTEGRATION (kept but bypassed for now)
// ============================================================
...
async function callVarifyApi(endpoint, data) {
  // This function is kept but we will bypass it.
  // If the endpoint becomes available, remove the bypass.
...
app.post('/api/deposit/request', async (req, res) => {
  ...
  try {
    // ----- BYPASS varify.et: create a manual reference -----
    const manualRef = 'manual-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    // We'll store the deposit as PENDING; admin must approve via /api/admin/deposits/approve
...
// ============================================================
// VARIFY.ET WEBHOOKS (kept for future use, but will rarely be called)
// ============================================================
app.post('/webhook/varify/deposit', async (req, res) => {
  // This endpoint is kept but since we bypass varify.et, it may not be used.
  // If a webhook arrives, we process it (but our manual deposits have varify_status='MANUAL' so we might ignore)
  const { reference, transaction_id, status, amount, method, phone } = req.body;
  console.log('Deposit webhook received (ignored unless status is for a manual tx):', req.body);
  ...
  try {
    ...
    if (deposit.varify_status === 'MANUAL') {
      // Manual deposits are not processed by varify.et; we ignore webhook for them.
...
// Admin approval endpoints (unchanged – they already approve/reject)
  ...
  ws.on('message', async (raw) => {
    try {
      ...
      switch (type) {
        ...
        // ---- WebSocket deposit/withdraw requests (now manual) ----
        case 'depositRequest': {
...
server.listen(config.PORT, async () => {
  console.log('============================================================');
  console.log('🎯 M-BINGO MODULAR SERVER (manual deposit/withdrawal mode)');
