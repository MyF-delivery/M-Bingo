...
// Make pool globally available for services
global.pool = pool;
...
// Health checks
app.get('/', (req, res) => res.json({ ok: true, service: 'm-bingo', status: 'online', websocket: true, health: '/health', version: '3.2.0' }));
...
function validStake(stake) { const n = Number(stake); return Number.isFinite(n) && [10,20,30,40,50,100].includes(n) ? n : null; }
function safeNumber(n) { const x = Number(n); return Number.isFinite(x) ? x : 0; }

// ---------- API Routes ----------
// (All routes from original app.js remain, but now call service functions)
// I will keep the exact same route handlers, only replacing internal logic with service calls.
...
app.post('/api/deposit/request', async (req, res) => {
  ...
  try {
    ...
    await notification.sendAdminNotification(`💰 New Deposit Request!\nUser: ${user.first_name} (${user.telegram_id})\nAmount: ${value} Birr\nMethod: ${method}\nTxn: ${txnId}\nStatus: Pending Approval`);
...
app.post('/api/withdraw/request', async (req, res) => {
  ...
  try {
    ...
    await notification.sendAdminNotification(`🏦 New Withdrawal Request!\nUser: ${user.first_name} (${user.telegram_id})\nAmount: ${value} Birr\nMethod: ${method}\nAccount: ${account}\nStatus: Pending Approval`);
...
app.post('/api/referral/process', async (req, res) => {
  ...
  try {
    ...
    const before = Number(referrerBalance.rows[0].balance); const after = before + config.REFERRAL_BONUS;
    await client.query(`UPDATE users SET balance = $1 WHERE id = $2`, [after, referrer.id]);
    await wallet.addLedger(client, referrer.id, 'REFERRAL_BONUS', config.REFERRAL_BONUS, before, after, 'REFERRAL', newUser.id);
    await client.query(`INSERT INTO referrals (referrer_id, referred_id, bonus_amount, status, paid_at) VALUES ($1, $2, $3, 'PAID', CURRENT_TIMESTAMP)`, [referrer.id, newUser.id, config.REFERRAL_BONUS]);
...
app.post('/api/admin/action', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (action === 'deposit') {
      ...
      res.json({ success: true, newBalance: (await pool.query(`SELECT balance FROM users WHERE id=$1`, [target.id])).rows[0].balance });
...
// Override game's broadcast functions to use the WebSocket map
        ...
        case 'startSelection': {
          ...
          try {
            ...
            if (!joined.rows.length) {
              await client.query(`INSERT INTO room_players(room_id,user_id,cards,is_ready) VALUES($1,$2,'[]'::jsonb,FALSE)`, [room.id, user.id]);
            }
          ...
          try {
            ...
            if (snapshot) {
              ...
              broadcastToRoom(ws.roomId, 'gameStateUpdate', {
                ...
                gameNumber: snapshot.game_number,
                selectedCards,
                selectionTimeLeft: snapshot.state === 'SELECTING' ? Math.max(0, Math.ceil((new Date(snapshot.created_at).getTime() + Number(snapshot.countdown_seconds || config.BINGO_SELECTION_SECONDS) * 1000 - Date.now()) / 1000)) : 0,
                players: snapshot.players
              });
            }
