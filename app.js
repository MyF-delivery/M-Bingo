...
const axios = require('axios'); // <-- added for varify.et
...
const VARIFY_WEBHOOK_SECRET = process.env.VARIFY_WEBHOOK_SECRET; // optional for signature verification

async function callVarifyApi(endpoint, data) {
  if (!VARIFY_API_KEY) throw new Error('VARIFY_API_KEY is not set in environment');
  const response = await axios.post(`${VARIFY_API_URL}${endpoint}`, data, {
    headers: {
      'Authorization': `Bearer ${VARIFY_API_KEY}`,
...
// ============================================================
// DEPOSIT via VARIFY.ET
// ============================================================
app.post('/api/deposit/request', async (req, res) => {
  const { userId, amount, method, phone } = req.body; // phone is now required
  const user = await findUser(userId);
  if (!user) return res.status(404).json({ success: false, message: 'User not found' });
  const value = safeNumber(amount);
  if (value < 50 || value > 5000)
    return res.status(400).json({ success: false, message: 'Amount must be between 50 and 5000' });
  if (!phone) return res.status(400).json({ success: false, message: 'Phone number is required for deposit via varify.et' });

  try {
    // 1. Call varify.et to initiate deposit
    const varifyPayload = {
      amount: value,
      phone: phone,
      method: method || 'CBE',
      callback_url: `${process.env.BASE_URL || config.BASE_URL}/webhook/varify/deposit`,
    };
    const varifyResult = await callVarifyApi(VARIFY_DEPOSIT_ENDPOINT, varifyPayload);

    // 2. Store deposit record with varify reference
    const deposit = await pool.query(
      `INSERT INTO deposits (user_id, amount, method, reference, varify_reference, varify_status, status)
       ...
       RETURNING *`,
      [user.id, value, method, varifyResult.reference || null, varifyResult.transaction_id || null]
    );

    // 3. Notify admin (optional)
    await notification.sendAdminNotification(
      `💰 Deposit initiated via varify.et\nUser: ${user.first_name} (${user.telegram_id})\nAmount: ${value} Birr\nMethod: ${method}\nPhone: ${phone}\nReference: ${varifyResult.reference}`
    );

    res.json({ success: true, deposit: deposit.rows[0], varify_reference: varifyResult.reference });
  } catch (error) {
    console.error('Deposit varify.et error:', error.response?.data || error.message);
    res.status(500).json({ success: false, message: 'Failed to initiate deposit with varify.et' });
  }
});

// ============================================================
// WITHDRAWAL via VARIFY.ET
// ============================================================
app.post('/api/withdraw/request', async (req, res) => {
  const { userId, amount, method, account } = req.body; // account = phone number
  const user = await findUser(userId);
  if (!user) return res.status(404).json({ success: false, message: 'User not found' });
  const value = safeNumber(amount);
  if (value <= 0) return res.status(400).json({ success: false, message: 'Invalid amount' });
  if (!account) return res.status(400).json({ success: false, message: 'Phone number (account) is required for withdrawal' });
  ...
  try {
    ...
    // 2. Call varify.et to initiate withdrawal
    const varifyPayload = {
      amount: value,
      phone: account,
      method: method || 'CBE',
      callback_url: `${process.env.BASE_URL || config.BASE_URL}/webhook/varify/withdraw`,
    };
    const varifyResult = await callVarifyApi(VARIFY_WITHDRAW_ENDPOINT, varifyPayload);

    // 3. Update withdrawal with varify reference
    await pool.query(
      `UPDATE withdrawals SET varify_reference = $1, varify_status = 'PENDING' WHERE id = $2`,
      [varifyResult.transaction_id || varifyResult.reference, withdrawal.rows[0].id]
    );

    await notification.sendAdminNotification(
      `🏦 Withdrawal initiated via varify.et\nUser: ${user.first_name} (${user.telegram_id})\nAmount: ${value} Birr\nMethod: ${method}\nAccount: ${account}\nReference: ${varifyResult.reference}`
    );

    res.json({ success: true, withdrawal: withdrawal.rows[0], varify_reference: varifyResult.reference });
  } catch (error) {
    await client.query('ROLLBACK').catch(() => {});
    console.error('Withdrawal varify.et error:', error.response?.data || error.message);
    res.status(500).json({ success: false, message: 'Failed to initiate withdrawal with varify.et' });
  } finally {
    client.release();
  }
});
...
app.post('/webhook/varify/deposit', async (req, res) => {
  // Optional: verify signature using VARIFY_WEBHOOK_SECRET
  const signature = req.headers['x-varify-signature'];
  // You can implement signature verification here

  const { reference, transaction_id, status, amount, method, phone } = req.body;
  console.log('Deposit webhook received:', req.body);
  ...
  try {
    ...
    try {
      ...
      if (status === 'COMPLETED' || status === 'SUCCESS') {
        ...
        const userWs = clients.get(String(deposit.user_id));
        if (userWs) {
          send(userWs, 'depositStatus', { status: 'confirmed', amount: deposit.amount, newBalance: after });
        }
      } else {
        // Failed / rejected
        await client.query(
        ...
        const userWs = clients.get(String(deposit.user_id));
        if (userWs) {
          send(userWs, 'depositStatus', { status: 'rejected', message: 'Deposit failed via varify.et' });
        }
      }
      await client.query('COMMIT');
    } catch (e) {
...
app.post('/webhook/varify/withdraw', async (req, res) => {
  const { reference, transaction_id, status, amount, method, phone } = req.body;
  console.log('Withdrawal webhook received:', req.body);
  ...
  try {
    ...
    try {
      await client.query('BEGIN');
      if (status === 'COMPLETED' || status === 'SUCCESS') {
        // Withdrawal already deducted from balance and reserved; just update status
        await client.query(
          `UPDATE withdrawals SET status = 'APPROVED', varify_status = 'COMPLETED', approved_at = CURRENT_TIMESTAMP WHERE id = $1`,
          [withdrawal.id]
        );
        // Notify user
        const userWs = clients.get(String(withdrawal.user_id));
        if (userWs) {
          send(userWs, 'withdrawStatus', { status: 'confirmed', amount: withdrawal.amount });
        }
      } else {
        // Failed; refund reserved balance
        await client.query(
        ...
        const userWs = clients.get(String(withdrawal.user_id));
        if (userWs) {
          send(userWs, 'withdrawStatus', { status: 'rejected', message: 'Withdrawal failed via varify.et' });
        }
      }
      await client.query('COMMIT');
    } catch (e) {
...
// Admin approval endpoints (kept for manual override if needed)
...
// ---------- Lobby Data Broadcast (unchanged) ----------
async function broadcastLobbyData() {
  try {
    const result = await pool.query(`
      ...
      SELECT 
        stake,
        SUM(selected_cards) AS total_selected_cards,
    ...
    const data = allStakes.map(stake => {
      ...
      if (!found) {
        return { stake, selectedCardsCount: 0, playersCount: 0, status: 'waiting', selectionTimeLeft: 0 };
      }

      let status = 'waiting';
      let selectionTimeLeft = 0;

      if (Number(found.has_playing) > 0) {
        status = 'playing';
    ...
    const payload = { type: 'lobbyData', data: data };
    for (const ws of clients.values()) {
    ...
    console.error('broadcastLobbyData error:', e);
  }
}
    ...
    try {
      ...
      switch (type) {
        ...
        case 'getGameState': {
          ...
          if (snapshot) {
            const selectedCards = snapshot.players.flatMap((p) => p.cards || []);
            const cards = snapshot.state === 'PLAYING' ? await game.getGameCards(ws.roomId, snapshot.called_numbers || []) : [];
        ...
        case 'getLobbyData': {
          await broadcastLobbyData();
          break;
        }

        // WebSocket deposit request (optional, can use API route instead)
        case 'depositRequest': {
          const { userId, amount, method, phone } = data;
          // Re-use the API logic, but we can call the route internally or forward.
          // For simplicity, we forward to the API handler by creating a mock request.
          // Alternatively, we can call the function directly.
          // We'll implement the same logic here to avoid duplication.
          ...
          try {
            ...
            sendError(ws, err.message || 'Deposit initiation failed');
          }
          break;
        }
        ...
        case 'withdrawRequest': {
          ...
          try {
            ...
            await pool.query(
              `UPDATE withdrawals SET varify_reference = $1, varify_status = 'PENDING' WHERE id = $2`,
              [varifyResult.transaction_id || varifyResult.reference, withdrawal.id]
            );
            send(ws, 'withdrawStatus', { status: 'pending', withdrawal: withdrawal });
          } catch (err) {
            sendError(ws, err.message || 'Withdrawal initiation failed');
          }
          break;
        }
...
server.listen(config.PORT, async () => {
  console.log('============================================================');
  console.log('🎯 M-BINGO MODULAR SERVER (varify.et integrated)');
