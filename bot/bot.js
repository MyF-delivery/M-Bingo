...
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || ''; // Add plain password support
...
async function getOrCreateUser(telegramId, firstName, lastName, username, referralCode) {
    try {
        const response = await axios.post(`${API_URL}/api/users/register`, {
            telegramId, username: username || `user_${telegramId}`, firstName, lastName: lastName || '', referralCode: referralCode || null
        }, { headers: { 'x-bot-token': BOT_TOKEN } });
        return response.data.user;
    } catch (error) { console.error('Reg error:', error.response?.data?.message || error.message); return null; }
}

async function getUserBalance(telegramId) {
    try { const r = await axios.get(`${API_URL}/api/wallet/${telegramId}`); return r.data.balance || 0; }
    catch (error) { return 0; }
}

function formatCurrency(amount) { return amount.toLocaleString('en-US') + ' ETB'; }
...
bot.onText(/\/start(?:\s+ref_(\d+))?/, async (msg, match) => {
    const chatId = msg.chat.id; const userId = msg.from.id; const firstName = msg.from.first_name || 'Player';
    const lastName = msg.from.last_name || ''; const username = msg.from.username || `user_${userId}`;
    ...
    try {
        ...
        if (r.data.user) {
            ...
            await bot.sendMessage(chatId, `👋 *Welcome back, ${firstName}!*\n\n💰 *Balance:* ${formatCurrency(balance)}\n👇 *Select an option:*`, { parse_mode: 'Markdown', ...mainMenu(userId) });
            return;
        }
    } catch (e) {}

    const registerKeyboard = { reply_markup: { keyboard: [[{ text: "📱 Share Contact", request_contact: true }]], resize_keyboard: true, one_time_keyboard: true } };
    ...
    bot._referralMap = bot._referralMap || {}; bot._referralMap[userId] = referralId;
});

bot.on('contact', async (msg) => {
    const chatId = msg.chat.id; const userId = msg.from.id; const contact = msg.contact;
    const firstName = msg.from.first_name || 'Player'; const lastName = msg.from.last_name || ''; const username = msg.from.username || `user_${userId}`;
    if (!contact || contact.user_id != userId) return bot.sendMessage(chatId, '❌ Please share your own contact.');

    try { const r = await axios.get(`${API_URL}/api/users/${userId}`); if (r.data.user) { await bot.sendMessage(chatId, '✅ You are already registered!'); await bot.sendMessage(chatId, '🎯 *Welcome back!*', { parse_mode: 'Markdown', ...mainMenu(userId) }); return; } } catch (e) {}
    ...
    if (user) {
        if (referralId) { try { await axios.post(`${API_URL}/api/referral/process`, { referrerId: referralId, newUserId: userId }); await bot.sendMessage(referralId, `🎉 You earned ${REFERRAL_BONUS} Birr bonus!`); } catch (e) {} }
...
bot.onText(/\/balance/, async (msg) => {
    const chatId = msg.chat.id; const userId = msg.from.id; const balance = await getUserBalance(userId);
...
bot.onText(/\/bonus/, async (msg) => {
    ...
    await bot.sendMessage(chatId, `🎁 *Invite Friends & Earn!*\n\n${inviteLink}\n\nEarn *${REFERRAL_BONUS} Birr* per friend!`, { parse_mode: 'Markdown' });
});
...
bot.onText(/\/profile/, async (msg) => {
    const chatId = msg.chat.id; const userId = msg.from.id;
    try { const r = await axios.get(`${API_URL}/api/users/${userId}`); const user = r.data.user; await bot.sendMessage(chatId, `👤 *Profile*\n\n📛 Name: ${user.first_name}\n🆔 ID: ${user.telegramId}\n💰 Balance: ${user.balance} ETB\n🏆 Wins: ${user.wins || 0}`, { parse_mode: 'Markdown' }); } catch (e) { await bot.sendMessage(chatId, '⚠️ Could not fetch profile.'); }
});

// Callback Handler (Play, Admin, and Emit commands)
bot.on('callback_query', async (call) => {
    const chatId = call.message.chat.id; const userId = call.from.id; const data = call.data;
    bot.answerCallbackQuery(call.id);

    if (data.startsWith('stake_')) {
        const stake = parseInt(data.split('_')[1]);
        const gameUrl = `${GAME_URL}?stake=${stake}&userId=${userId}`;
        await bot.sendMessage(chatId, `✅ Stake set to ${stake} Birr.\n\n🎮 Click below to select your cards:`, { reply_markup: { inline_keyboard: [[{ text: "🎮 Open Game", web_app: { url: gameUrl } }]] } });
        return;
    }
...
// Text Handler (Admin Login & Flows)
bot.on('message', async (msg) => {
    ...
    if (userState[chatId]?.step === 'DEPOSIT_AMOUNT') {
        const amount = parseInt(text); if (isNaN(amount) || amount < 50 || amount > 5000) return bot.sendMessage(chatId, '❌ Invalid amount. Enter between 50 and 5000.');
        userState[chatId] = { step: 'DEPOSIT_METHOD', amount };
        return bot.sendMessage(chatId, '💰 *Select method:*', { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: "🏦 Bank Transfer", callback_data: `deposit_method_bank_${amount}` }, { text: "💳 CBE Birr", callback_data: `deposit_method_cbe_${amount}` }]] } });
    }

    if (userState[chatId]?.step === 'WITHDRAW_AMOUNT') {
        const amount = parseInt(text); const balance = await getUserBalance(userId);
        ...
        return bot.sendMessage(chatId, '📤 *Select method:*', { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: "🏦 Bank Transfer", callback_data: `withdraw_method_bank_${amount}` }, { text: "💳 CBE Birr", callback_data: `withdraw_method_cbe_${amount}` }]] } });
    }

    if (userState[chatId]?.step === 'TRANSFER_PHONE') {
        const recipient = text.trim();
        try { await axios.get(`${API_URL}/api/users/${recipient}`); userState[chatId] = { step: 'TRANSFER_AMOUNT', recipient }; return bot.sendMessage(chatId, '💵 *Enter amount to transfer:*', { parse_mode: 'Markdown' }); }
        catch (e) { return bot.sendMessage(chatId, '❌ User not found. Please try again.'); }
    }
...
// Deposit/Withdraw Methods & Admin Actions (Second Callback Handler)
bot.on('callback_query', async (call) => {
    const chatId = call.message.chat.id; const userId = call.from.id; const data = call.data;
    bot.answerCallbackQuery(call.id);

    if (data.startsWith('deposit_method_')) {
        const parts = data.split('_'); const method = parts[2]; const amount = parseInt(parts[3]);
    ...
    if (data === 'admin_players') { // Player List Logic
        if (userId.toString() !== ADMIN_TELEGRAM_ID) return bot.sendMessage(chatId, '⛔ Unauthorized.');
        try { const r = await axios.get(`${API_URL}/api/admin/players`); const players = r.data; let msg = '👥 *Players List*\n\n'; players.slice(0, 20).forEach((p, i) => msg += `${i+1}. ${p.first_name} - 💰 ${p.balance} ETB\n`); await bot.sendMessage(chatId, msg, { parse_mode: 'Markdown' }); } catch (e) { await bot.sendMessage(chatId, '⚠️ Error fetching players.'); }
        return;
    }
    if (data === 'admin_deposits' || data === 'admin_withdrawals') {
        if (userId.toString() !== ADMIN_TELEGRAM_ID) return bot.sendMessage(chatId, '⛔ Unauthorized.');
        const isDeposit = data === 'admin_deposits'; const url = isDeposit ? `${API_URL}/api/admin/deposits` : `${API_URL}/api/admin/withdrawals`;
        try { const r = await axios.get(url); const items = r.data; if (!items || items.length === 0) return bot.sendMessage(chatId, 'No pending items.'); let msg = isDeposit ? '📥 *Pending Deposits*\n\n' : '📤 *Pending Withdrawals*\n\n'; items.forEach((item) => { msg += `👤 ${item.userName} (${item.userId})\n💰 ${formatCurrency(item.amount)}\n🆔 ${item.txnId || item.account}\n\n`; }); await bot.sendMessage(chatId, msg, { parse_mode: 'Markdown' }); } catch (e) { await bot.sendMessage(chatId, '⚠️ Error fetching items.'); }
        return;
    }
    if (data === 'admin_stats') {
        if (userId.toString() !== ADMIN_TELEGRAM_ID) return bot.sendMessage(chatId, '⛔ Unauthorized.');
        try { const r = await axios.get(`${API_URL}/api/admin/stats`); const s = r.data; await bot.sendMessage(chatId, `📊 *Stats*\n\n👥 Players: ${s.totalPlayers}\n🎮 Active Games: ${s.activeGames}`, { parse_mode: 'Markdown' }); } catch (e) { await bot.sendMessage(chatId, '⚠️ Error fetching stats.'); }
        return;
    }
    if (data === 'back_to_menu') { await bot.sendMessage(chatId, '🎯 *Welcome back!*', { parse_mode: 'Markdown', ...mainMenu(userId) }); return; }
});

// Txn & Account handlers
bot.on('message', async (msg) => {
    const chatId = msg.chat.id; const text = msg.text; if (!text) return;
    if (userState[chatId]?.step === 'DEPOSIT_TXN') {
        const txnId = text.trim(); const state = userState[chatId];
        await axios.post(`${API_URL}/api/deposit/request`, { userId: state.userId, amount: state.amount, method: state.method, txnId });
        await bot.sendMessage(chatId, `✅ Deposit request submitted! Waiting for admin confirmation.`);
        await bot.sendMessage(ADMIN_TELEGRAM_ID, `💳 Deposit: ${state.amount} ETB from ${state.userId} (Txn: ${txnId})`);
        delete userState[chatId]; return;
    }
    if (userState[chatId]?.step === 'WITHDRAW_ACCOUNT') {
        const account = text.trim(); const state = userState[chatId];
        await axios.post(`${API_URL}/api/withdraw/request`, { userId: state.userId, amount: state.amount, method: state.method, account });
        await bot.sendMessage(chatId, `✅ Withdrawal request submitted! Waiting for admin confirmation.`);
        await bot.sendMessage(ADMIN_TELEGRAM_ID, `📤 Withdrawal: ${state.amount} ETB from ${state.userId} (Account: ${account})`);
        delete userState[chatId]; return;
    }
});

app.listen(PORT, '0.0.0.0', () => console.log(`🌐 Admin API on port ${PORT}`));
bot.setChatMenuButton({ menu_button: { type: 'web_app', text: '🎮 Play M-BINGO', web_app: { url: GAME_URL } } }).catch(() => {});
