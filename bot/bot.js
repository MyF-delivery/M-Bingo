// ============================================================
// M-BINGO TELEGRAM BOT - FULL FEATURED (FINAL FIXED VERSION)
// ============================================================
...
bot.on('contact', async (msg) => {
    ...
    // Check again if already registered to prevent duplicate errors
    try {
        const response = await axios.get(`${API_URL}/api/users/${userId}`);
        if (response.data.user) {
...
setInterval(async () => {
    try {
        // Poll for approved/rejected deposits
        ...
        // Poll for approved/rejected withdrawals
