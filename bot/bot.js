...
// /register - Trigger manual registration (if needed)
bot.onText(/\/register/, async (msg) => {
    ...
    // Re-trigger the start flow (share contact)
    const registerMessage = `
📝 *To register, please share your contact.*
...
bot.on('callback_query', async (call) => {
    ...
    switch (data) {
        ...
        case 'deposit':
            bot.emit('text', { chat: { id: chatId }, from: { id: userId } }, '/deposit');
            break;

        case 'withdraw':
            bot.emit('text', { chat: { id: chatId }, from: { id: userId } }, '/withdraw');
            break;

        case 'transfer':
            bot.emit('text', { chat: { id: chatId }, from: { id: userId } }, '/transfer');
            break;

        case 'bonus':
            bot.emit('text', { chat: { id: chatId }, from: { id: userId } }, '/bonus');
            break;

        case 'history':
            bot.emit('text', { chat: { id: chatId }, from: { id: userId } }, '/history');
            break;

        case 'help':
            bot.emit('text', { chat: { id: chatId }, from: { id: userId } }, '/help');
            break;
