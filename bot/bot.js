...
bot.onText(/\/start(?:\s+ref_(\d+))?/, async (msg, match) => {
    ...
    try {
        // ✅ AUTO-REGISTER: Check if user exists, if not create them
        ...
        // Check if user was newly registered or already existed
        // If the user was just created, they won't have a gamesPlayed field or it will be 0
        if (response.data.message && response.data.message.includes('success')) {
            isNewUser = !userData.gamesPlayed || userData.gamesPlayed === 0;
        }
        ...
        // If registration fails, try to get existing user data
        try {
            const getUserResponse = await axios.get(`${API_URL}/api/users/${userId}`, {
                headers: { 'x-bot-token': BOT_TOKEN }
            });
            userData = getUserResponse.data.user;
        } catch (getError) {
            console.log('⚠️ Could not fetch user data:', getError.message);
        }
    }
    
    // Build welcome message based on new or existing user
    let welcomeText;
    if (isNewUser) {
        welcomeText = `
🎯 *Welcome to M-BINGO, ${firstName}!* 🎉

💰 *You start with 0 Birr bonus!*
...
👇 *Tap the button below to open the game:*
    ...
    // Add a dedicated "Play Now" button inside the message
...
bot.onText(/\/withdraw/, async (msg) => {
    ...
    bot.sendMessage(chatId, "📤 Please contact @frezerabiy to process your withdrawal.");
});
...
🎖️ *Wins:* ${user.wins || 0}
📈 *Total Winnings:* ${user.totalWinnings || 0} Birr
📅 *Joined:* ${new Date(user.createdAt).toLocaleDateString()}
        `;
        
        bot.sendMessage(chatId, message, { parse_mode: 'Markdown' });
    } catch (error) {
        bot.sendMessage(chatId, '⚠️ Could not fetch profile. Please play a game first.');
    }
});
...
// ============================================================
// TELEGRAM WEB APP MENU BUTTON
// ============================================================

async function setupWebAppMenu() {
    try {
        ...
        console.error(
            '⚠️ Failed to configure Telegram Web App menu button:',
            error.message
        );
    }
}
